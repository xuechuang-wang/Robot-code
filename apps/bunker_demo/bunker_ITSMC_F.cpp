...
int main(int argc, char **argv)
{
    ...
    double c1 = 0.9;      //? 控制器增益
    double c2 = 0.6;      //? 控制器增益  减小可降ye超调 26
    // //! SMO
    double k1 = 0.001;      //? 控制器增益
    double k2 = 0.001;      //? 控制器增益


    double alpha1 = 0.6;
    double alpha2 = 0.7;  //? 终端指数   减小可加快ye初始收敛速度
    ...
    while (true)
    {
        ...
        m1 += c1 * signxE * pow(fabs(xE),alpha1) * Ts;
        m2 += c2 * signyE * pow(fabs(yE),alpha2) * Ts;

        s1 = m1 + xE;
        s2 = m2 + yE;


        u1 = dotXR + c1 * signxE * pow(fabs(xE),alpha1) + k1 * signs1 - z12;
        u2 = dotYR + c2 * signyE * pow(fabs(yE),alpha2) + k2 * signs2 - z22;
